<template>
    <div>
      ...
      <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
        <el-form-item>
          <el-input v-model="dataForm.roleName" placeholder="订单号" clearable></el-input>
        </el-form-item>
        <el-form-item>
      ...
      <el-table ref="list" :data="tableData" style="width: 100%" border stripe highlight-current-row
        ...
        <el-table-column property="oid" label="订单号">
        </el-table-column>
        <el-table-column property="startDate" label="开始日期">
        </el-table-column>
        <el-table-column property="deliveryDate" label="交货日期">
        </el-table-column>
        <el-table-column property="planid" label="生产计划号">
        </el-table-column>
      ...
      <el-dialog title="编辑" :visible.sync="isShowEditDialog" width="445px" @close="dialogClose">
        <el-form ref="editForm" :model="formFileds" label-width="70px" :rules="rules">
          <el-form-item label="订单号">
            <el-input v-model="formFileds.oid"></el-input>
          </el-form-item>
          <el-form-item label="开始日期">
            <el-input v-model="formFileds.startDate"></el-input>
          </el-form-item>
          <el-form-item label="交货日期">
            <el-input v-model="formFileds.deliveryDate"></el-input>
          </el-form-item>
          <el-form-item label="生产计划号">
            <el-input v-model="formFileds.planid"></el-input>
          </el-form-item>
          <el-form-item>
    ...
    methods: {
      ...
      deleteHandle() {
        ...
        for (var i = 0; i < this.dataListSelections.length; i++) {
          this.PositionId.push(this.dataListSelections[i].oid)
        }
      ...
      getDataList() {
        if (this.dataForm.roleName == '') {
          ...
          this.$axios({
            method: 'post',
            url: '/FindOedersById/' + this.dataForm.roleName
          }).then(res => {
            if (res.data.message == null) {
              ...
              this.tableData[0].oid = this.dataForm.roleName
            }
      ...
      isvalue() {
        if (this.$store.state.PositionForm.orders == '更改') {
          return true;
        } else {
          return false;
        }
      ...
      handleEdit(id) {
        console.log(id)
        if (id == true) {
          let newFormposition = this.formFileds.oid;
          let newForm = {};
      ...
      rowDel(index, row, event) {
        ...
        this.$confirm('确定要删除当前行吗？', '删除', {
          ...
          this.PositionForm = [];
          this.PositionForm.push(this.tableData[index].oid);
          this.$axios({
            method: 'post',
            url: '/DeleteOrdersById',

<template>
    <div>
      ...
      <el-form :inline="true" :model="dataForm" @keyup.enter.native="getDataList()">
        <el-form-item>
          <el-input v-model="dataForm.roleName" placeholder="日志号" clearable></el-input>
        </el-form-item>
        <el-form-item>
      ...
      <el-table ref="list" :data="tableData" style="width: 100%" border stripe highlight-current-row
        ...
        <el-table-column property="logid" label="日志号">
        </el-table-column>
        <el-table-column property="deviceId" label="设备号">
        </el-table-column>
        <el-table-column property="logDate" label="日志日期">
        </el-table-column>
        <el-table-column property="Content" label="日志内容">
        </el-table-column>
        <el-table-column property="Recorder" label="记录人">
        </el-table-column>
      ...
      <el-dialog title="编辑" :visible.sync="isShowEditDialog" width="445px" @close="dialogClose">
        <el-form ref="editForm" :model="formFileds" label-width="70px" :rules="rules">
          <el-form-item label="日志号">
            <el-input v-model="formFileds.logid"></el-input>
          </el-form-item>
          <el-form-item label="设备号">
            <el-input v-model="formFileds.deviceId"></el-input>
          </el-form-item>
          <el-form-item label="日志日期">
            <el-input v-model="formFileds.logDate"></el-input>
          </el-form-item>
          <el-form-item label="日志内容">
            <el-input v-model="formFileds.Content"></el-input>
          </el-form-item>
          <el-form-item label="记录人">
            <el-input v-model="formFileds.Recorder"></el-input>
          </el-form-item>
          <el-form-item>
    ...
    methods: {
      ...
      deleteHandle() {
        ...
        for (var i = 0; i < this.dataListSelections.length; i++) {
          this.PositionId.push(this.dataListSelections[i].logid)
        }
      ...
      getDataList() {
        if (this.dataForm.roleName == '') {
          ...
          this.$axios({
            method: 'post',
            url: '/logsByLogid/' + this.dataForm.roleName
          }).then(res => {
            if (res.data.message == null) {
              ...
              this.tableData[0].logid = this.dataForm.roleName
            }
      ...
      isvalue() {
        if (this.$store.state.PositionForm.logs == '更改') {
          return true;
        } else {
          return false;
        }
      ...
      handleEdit(id) {
        console.log(id)
        if (id == true) {
          let newFormposition = this.formFileds.logid;
          let newForm = {};
      ...
      rowDel(index, row, event) {
        ...
        this.$confirm('确定要删除当前行吗？', '删除', {
          ...
          this.PositionForm = [];
          this.PositionForm.push(this.tableData[index].logid);
          this.$axios({
            method: 'post',
